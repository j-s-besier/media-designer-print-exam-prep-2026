...
- [ ] 1.1 Create the `data/` directory structure for exam packages, attempts, results, schemas, templates, and assets.
...
- [ ] 1.5 Define `result.schema.json` for paper totals, task totals, subtask scores, feedback, confidence, manual-review flags, total points, and percentage.
- [ ] 1.6 Define `manifest.schema.json` for gallery metadata, exam identity, versioning, status, and content-policy metadata.
- [ ] 1.7 Create reusable JSON templates for a Mediengestalter Printmedien written exam package and solution package.

## 2. Exam Package Validation

- [ ] 2.1 Implement package validation that checks required files: `manifest.json`, `exam.json`, and `solution.json`.
- [ ] 2.2 Validate that written exams contain PB4, PB2, and PB3 in sequence and do not include PB1 in the written flow.
- [ ] 2.3 Validate that PB2 and PB3 each contain U1-U8 general blocks requiring 7 of 8 tasks and U9-U12 Printmedien blocks requiring 3 of 4 tasks.
- [ ] 2.4 Validate that every PB2/PB3 task has 10 points and that subtask points sum to the task maximum.
- [ ] 2.5 Validate that every answer field ID is unique and every attempt/solution reference points to an existing exam field or subtask.
- [ ] 2.6 Validate that `exam.json` contains no solution, rubric, model-answer, or awarded-point data.
- [ ] 2.7 Validate content-compliance metadata and flag packages with missing user-provided asset rights metadata.
...
- [ ] 3.4 Implement skill instructions to generate `manifest.json`, `exam.json`, `solution.json`, assets metadata, and validation output together.
...
- [ ] 4.5 Ensure the gallery never loads or exposes `solution.json`.
...
- [ ] 5.4 Implement text, list, table, calculation, file upload, and drawing/sketch upload answer fields.
...
- [ ] 6.2 Implement grading workflow instructions to load `exam.json`, `solution.json`, and a submitted attempt.
...
- [ ] 6.6 Aggregate subtask scores into task, paper, and overall totals.
- [ ] 6.7 Write `result.json` without mutating exam, solution, or attempt data.
- [ ] 6.8 Ensure result data supports gallery display of percentage and awarded/possible points.
...
- [ ] 7.5 Add grading tests that excluded tasks are ignored and totals are calculated from evaluated tasks only.
- [ ] 7.6 Add compliance tests that fail packages containing solution fields in `exam.json` or missing asset rights metadata.
- [ ] 7.7 Run an end-to-end sample: create exam package, render attempt, submit PB4/PB2/PB3, grade, and display result.
